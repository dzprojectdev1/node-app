...
videoApi.get('/othervideo/:otherId', checkAuth, function(req, res) {
    ...
    dbConn.query("SELECT * FROM tbl_match WHERE main_user_id=? AND other_user_id=? AND status IN (6,7) AND publish=1", [userId, otherId], function(error, matchResult, matchFields) {
        ...
        dbConn.query('SELECT cdn_id, cdn_filtered_id, content_type from tbl_video where user_id= ? and is_reply=0 and publish=1 order by created_date desc', [otherId], function (error, results, fields){
            if (error) return res.status(400).send({error: true, detail: error.code, message: error.sqlMessage});
...
videoApi.get('/getVideosByMatchId/:matchId', checkAuth, function(req, res) {
    ...
    dbConn.query('SELECT * FROM tbl_match WHERE id=? AND main_user_id=? OR other_user_id=?', [matchId, userId, userId], function(error, getResults, getFields) {
        ...
        dbConn.query('SELECT a.cdn_id, a.content_type, a.created_date, b.name, b.email_address, b.birth_date, b.gender FROM tbl_video a INNER JOIN tbl_user b ON a.user_id=b.id WHERE a.match_id=? AND a.is_reply=1 AND a.is_primary=1 AND a.publish=1', matchId, function(error1, videoResults, videoFields) {
            if (error1) return res.status(400).send({error: true, detail: error1.code, message: error1.sqlMessage});
...
videoApi.get('/getMatchedMyVideo', checkAuth, function(req, res) {
    ...
    dbConn.query('Select cdn_id, content_type from tbl_video where ' + whereCondition, [userId], function(error, results, fields) {
        if (error) return res.status(400).send({error: true, detail: error.code, message: error.sqlMessage});
...
videoApi.post('/getVideoForOther', checkAuth, function(req, res) {
    ...
    dbConn.query('Select a.cdn_id, a.content_type from tbl_video a inner join tbl_match b On a.match_id=b.id Where ' + whereCondition, [userId, userId, otherId], function(error, results, fields) {
        if (error) return res.status(400).send({error: true, detail: error.code, message: error.sqlMessage});
...
videoApi.post('/getVideoForMe', checkAuth, function(req, res) {
    ...
    var whereCondition = 'user_id=? and is_primary=1 and is_reply=0 and publish=1 limit 1';
    dbConn.query('Select cdn_id, content_type from tbl_video where ' + whereCondition, [otherId], function(error, results, fields) {
        if (error) return res.status(400).send({error: true, detail: error.code, message: error.sqlMessage});
